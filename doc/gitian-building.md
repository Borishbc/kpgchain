...
*Setup instructions for a Gitian build of KPG Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the KPG
...
- [Building KPG Core](#building-kpg-core)
...
Clone the git repositories for kpgchain and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/kunpengproject/kpgchain --recursive
git clone https://github.com/kunpengproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/kpgchain/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the kpgchain repository under 'Fetch and create inputs' to install sources which require
...
To build KPG Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the kpgchain repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/kpgchain/.git/
...
For example:
```bash
URL=https://github.com/laanwj/kpgchain.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit kpgchain=${COMMIT} --url kpgchain=${URL} ../kpgchain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit kpgchain=${COMMIT} --url kpgchain=${URL} ../kpgchain/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit kpgchain=${COMMIT} --url kpgchain=${URL} ../kpgchain/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-qtum:
./bin/gbuild --commit kpgchain=${COMMIT},cpp-eth-kpgchain=${ETHCOMMIT} --url kpgchain=${URL},cpp-eth-kpgchain=${ETHURL} ../kpgchain/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the kpgchain git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../kpgchain/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/kunpengproject/kpgchain-detached-sigs.git

BTCPATH=/some/root/path/kpgchain
SIGPATH=/some/root/path/kpg-detached-sigs

./bin/gbuild --url kpgchain=${BTCPATH},signature=${SIGPATH} ../kpgchain/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/kpg-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/kpg-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/kpg-osx-build.assert
```
...
[kunpengproject/gitian.sigs](https://github.com/kunpengproject/gitian.sigs/) repository, or if that's not possible create a pull
